...
class BybitConfigRequest(BaseModel):
    ...
    api_secret: str
    testnet: bool = True
    # Opsional: "demo" | "testnet" | "live". Kalau diisi, ini yang menentukan
    # server Bybit mana yang dipakai (menggantikan field `testnet` yang lama
    # dan ambigu). Kalau kosong, `testnet` lama tetap dipakai sebagai fallback.
...
def update_bot_config(config: BybitConfigRequest):
    return bot_logic.update_bybit_credentials(config.api_key, config.api_secret, config.testnet, config.mode)
